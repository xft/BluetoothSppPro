 ...
 * */
public final class BluetoothSppClient extends BtSppComm {
	/** 常量:输入输出模式为16进制值 */
	 ...
	 * 
	 * @param bOutIO_Mode
	 *            输出io模式 IO_MODE_HEX / IO_MODE_STRING
	 * @return void
	 * */
	...
	public int Send(String sData) {
		if (IO_MODE_HEX == this.mbtTxDMode) { // 16进制字符串转换成byte值
			if (CHexConver.isHexStr(sData))
				return sendData(CHexConver.hexToBytes(sData));
			else
				return 0; // 无效的HEX值
		} else { // 将字符串直接变为char的byte送出
			if (null != this.msCharsetName) {
				try { // 尝试做字符集转换
					return sendData(sData.getBytes(this.msCharsetName));
				} catch (UnsupportedEncodingException e) { // 字符集转换失败时使用默认字符集
					return sendData(sData.getBytes());
				}
			} else
				return sendData(sData.getBytes());
		}
	...
	public String Receive() {
		...
		if (null != btTmp) {
			if (IO_MODE_HEX == this.mbtRxDMode) // 16进制字符串转换成byte值
				return (CHexConver.bytesToHex(btTmp)).concat(" ");
			else
	...
	/**
	 * 接收设备数据，指令行模式（阻塞模式） 备注：即在接收时遇到终止字符后，才会输出结果，并在输出结果中会剔除终止符
	 * 
	...
	public String ReceiveStopFlg() {
		...
		btTmp = recvData_StopFlg(this.mbtEndFlg);
		if (null == btTmp)
			return null; // 无效的接收
		else {
			if (null == this.msCharsetName)
